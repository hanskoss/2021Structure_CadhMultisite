...
>>> spin system objects, data sets
In our python code framework, each spin systems is associated with a protein 
sequence position, protein name, state (A: folded; B: unfolded/RC; C: dimer; 
D,E…: other). Each of the spin system objects contains a list of data set 
objects which behave like dictionaries. These dictionaries can contain a 
variety of entries, typically several experimental conditions, intensity data 
and/or chemical shifts. During subsequent data processing, the number of 
entries can grow, for example due to a temperature correction of the chemical 
shifts. There can also be created non-experimental datasets generated by 
fitting procedures. In this way, the results of a variety of fitting 
procedures are stored in any spin system object.
...
class spinsystem:
"""define spinsystem class
structure of the object will be:
spinsystem[n].datasets[m].datasetsvariables
spinsystem[n].spinsystemsvariables
"""
    def __init__(self):
        self.name = []
        self.ssnumlist = []
...
"""
create spinsystem objects, one for each residue + state
state defined as letter
A: folded
B: RC
C: dimer folded
D... others

Filetype 4 is not relevant for relaxation dispersion data
"""
...
"""Special function to estimate the error of Rex experiments.
"""
...
def adddata(spinsystems,spinlistpath,filetype,dataposition,setselect,setlabel,pathlist):
    ...
    if filetype ==2: #standard nitrogen-CPMG data
        ...
        if int(round(5*field,0)) == 6 and setprotoncpmg == 1:
            vc[19]=999999
...
class RDset:
    ...
    def addshifts(self,xlabel,spinsystems,trosy,field,ss,dsn,filetype):
        ...
        if trosy[0] == 'T' or trosy[0] == 'S':
            ...
            trshiftn=0
        if found == 1:
            self.hshiftav=np.average(hdat)
            self.nshiftav=np.average(ndat)
